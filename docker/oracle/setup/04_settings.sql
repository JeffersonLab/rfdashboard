...
ALTER SESSION SET CURRENT_SCHEMA = RFGRADTEAM_OWNER;

CREATE TABLE RFGRADTEAM_OWNER.SETTING
(
...
insert into SETTING (KEY, VALUE, TYPE, DESCRIPTION, TAG, WEIGHT) values ('NOTIFICATION_LINK_URL', 'https://github.com/JeffersonLab/rfdashboard', 'STRING', 'Notification link URL', 'NOTIFICATION', 4);
...
insert into SETTING (KEY, VALUE, TYPE, DESCRIPTION, TAG, WEIGHT) values ('HELP_DOC_CSV', 'https://github.com/JeffersonLab/rfdashboard|Home', 'CSV', 'CSV of documentation items where each item is a URL and a Label separated with the pipe symbol', 'HELP', 3);

-- OPTIONAL: Email
insert into SETTING (KEY, VALUE, TYPE, DESCRIPTION, TAG, WEIGHT, CHANGE_ACTION_JNDI) values ('EMAIL_ENABLED', 'N', 'BOOLEAN', 'Email integration enabled?', 'EMAIL', 0, 'java:/mail/jlab');
...
insert into SETTING (KEY, VALUE, TYPE, DESCRIPTION, TAG, WEIGHT) values ('EMAIL_SENDER_ADDRESS', 'rfdashboard@jlab.org', 'STRING', 'Email address to use as sender from emails generated in this app.  Note: this is not the same as "from".', 'EMAIL', 4);
